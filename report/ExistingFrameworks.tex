...
Note that the \dops{} extraction needs another decision: in many cases, there are several shortest derivations possible. We can choose to add all fragments that occur in these derivations to the symbolic grammar. If we do so, we might need to adjust the weights (e.g. divide the frequency counts by the number of derivations) so that no full tree gets a higher impact on the PTSG.
We will keep to the original formulation of \dops{} in case no derivation is possible, i.e. not including any fragments for this tree.

\paragraph{Extraction}
It is easy to see that the \dops{} extraction method does not depend on the corpus split: we can also try to find the shortest possble derivation using fragments from all the other trees. %The following example shows that the set of extracted fragments is indeed different from the maximal overlapping fragments \ddop{} extracts.
Therefore, we will implement both extraction methods in a 1 vs the rest manner and possibly also the split-based extraction.
In this way, we can analyse how the resulting symbolic grammars differ. The analysis will comprise the size of the resulting symbolic grammar and the relative number of fragments of certain depth. Furthermore, we might be able to find interesting patterns by manually looking at the fragments that were extracted by one of the systems only.s

\paragraph{Estimation}
If we have enough time, we can also compare the estimation methods: use either a split or the whole set of trees for both estimators. The comparison would involve a performance measure in this case, such as the F1-score for correctly predicted parses.
